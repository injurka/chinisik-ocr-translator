...
import { ALL_KEYS, STORAGE_KEY_CONTROLS } from '~/shared/constant'
...
async function loadControls() {
  try {
    ...
    if (result[STORAGE_KEY_CONTROLS]) {
      ...
      sharedControls.value = { ...sharedControls.value, ...loadedControls }
    }
  }
...
async function saveControls(newControls: ControlValues) {
  ...
  catch (e) { console.error('Error saving controls:', e) }
}
...
watch(sharedControls, saveControls, { deep: true })
...
async function handleSelection(area: AreaToCapture) {
  ...
  try {
    await browser.runtime.sendMessage({ action: 'captureAndTranslate', area })
  }
  catch (error) {
...
const getHostElement = () => document.getElementById('chinisik-ocr-translator-shadow-host')
...
function applyTheme(theme: Theme) {
  ...
  // Удаляем все классы тем, чтобы избежать их накопления
  host.classList.remove('dark', 'rainy')
  // Добавляем актуальный класс, если это не тема по умолчанию
  if (theme !== 'light')
...
async function loadAndApplyTheme() {
  try {
    ...
    const themeToApply = (result.appTheme as Theme) || 'light'
...
<template>
  <!-- Телепортируем оверлей для выделения в <body> страницы. -->
  <!-- Он будет отображаться только когда currentState === 'SELECTING'. -->
  <Teleport to="body">
    <Selection v-if="currentState === 'SELECTING'" v-model="isSelectionOverlayVisible" @handle-selection="handleSelection" />
  </Teleport>

  <!-- Основной UI, который будет находиться внутри Shadow DOM для изоляции стилей. -->
  <div>
